...
To open or close a modal, find the element by its selector (in this case,
`#dialog`) and call the corresponding `showModal()` and `close()` methods. 
The method calls would be `document.getElementById('dialog').showModal();` and
`document.getElementById('dialog').close();` in this example.

Make sure to add a click event listener to the modal content and call
